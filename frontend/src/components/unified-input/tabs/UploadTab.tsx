...
function UploadTab({ doCreate, stack, setStack, designSystem }: Props) {
  ...
  const dropzoneClassName = useMemo(() => {
    const base =
      "flex flex-1 w-full min-h-[320px] flex-col items-center justify-center p-5 border-2 border-dashed rounded-xl bg-white/5 text-gray-400 outline-none transition-all cursor-pointer";
    if (isFocused) {
      return `${base} border-purple-500 bg-purple-500/10`;
    }
    if (isDragAccept) {
      return `${base} border-green-400 bg-green-500/10`;
    }
    if (isDragReject) {
      return `${base} border-red-400 bg-red-500/10`;
    }
    return `${base} border-white/20 hover:border-purple-400/50`;
          ...
          <div className="relative w-full max-w-3xl">
            {uploadedInputMode === "video" ? (
              <div className="relative rounded-lg border border-white/10 bg-white/5 p-3">
                <video
                  src={files[0]?.preview}
                  className="w-full h-auto max-h-[400px] object-contain rounded-md border border-white/10"
                  controls
                />
                <button
                  onClick={handleClear}
                  className="absolute top-2 right-2 bg-white/10 rounded-full p-1.5 shadow-md hover:bg-white/20 transition-colors"
              ...
              <div
                {...getRootProps({
                  className: `relative rounded-lg border border-white/10 bg-white/5 p-4 ${
                    isDragActive ? "ring-2 ring-purple-500/30" : ""
                  }`,
                })}
              >
                <input {...getInputProps()} />
                  ...
                  {files.map((file, index) => (
                    <div key={`${file.name}-${index}`} className="relative group flex-shrink-0">
                      ...
                      <button
                        ...
                        className="absolute -top-1 -right-1 h-4 w-4 bg-black/60 hover:bg-red-500 text-white rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity"
